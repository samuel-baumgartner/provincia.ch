...
- Local Reddit automation uses Playwright + a persistent profile (`pnpm reddit:login` → `data/reddit-browser-profile/`), then `pnpm reddit:scan` / `pnpm reddit:engage`. Engage stays off until master+Reddit toggles in `/admin` (or env). New/low-trust accounts stay in **helpful-only** mode (no promo); once warmed and comments are visible logged-out, ~every 4th reply may be promo (3:1). Invisible comments soft-halt promos only — they do not stop the bot.
- Admin control center: `/admin` (health + kill switches + jobs), `/admin/setup` (channel checklists), `/admin/itch` (DevTalk → itch BBCode). Controls file: `reports/marketing-controls.json`.
