...
type Props = {
  slug: string;
  /** Relative path e.g. /devtalks/.../cover.png — used for immediate download when known */
  ...
  async function copyAndDownload() {
    ...
    try {
      ...
      try {
        ...
        // Clipboard can fail on insecure contexts; still download
      }

      const fileRes = await fetch(path.startsWith("http") ? path : path);
      if (!fileRes.ok) throw new Error("Could not download cover file");
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <button
        ...
        title={coverPath === null ? "Missing coverImage" : "Copy absolute URL and download the file"}
      ...
      >
        {state === "done" ? "URL copied + downloaded" : "Copy cover image"}
      </button>
